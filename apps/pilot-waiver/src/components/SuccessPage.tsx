...
export default function SuccessPage({ submission, onDownloadPDF, isGeneratingPDF }: SuccessPageProps) {
  const formattedExpiry = submission.expiryDate
    ? new Date(submission.expiryDate).toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' })
    : '—';
  const formattedSubmitted = submission.submittedAt
    ? new Date(submission.submittedAt).toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' })
    : '—';
    ...
    <div className="px-6 py-12 text-center space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-gray-600">
          Thank you, <strong>{submission.pilot.firstName} {submission.pilot.lastName}</strong>.
          Your pilot waiver has been recorded successfully.
        </p>
      </div>
      ...
      <Button variant="primary" onClick={onDownloadPDF} loading={isGeneratingPDF} className="mx-auto">
